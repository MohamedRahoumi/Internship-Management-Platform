    ...
    <div>
      ...
      {interns.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {interns.map((intern) => (
            <div key={intern.id} className="bg-white border border-slate-200 rounded-md p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500 truncate">{intern.user?.email}</p>
                </div>
              </div>
              <div className="space-y-2 text-sm border-t border-slate-100 pt-3">
                <div className="flex justify-between">
                  <span className="text-slate-500">CIN</span>
                  <span className="text-slate-800 font-medium">{intern.user?.cin || '—'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Téléphone</span>
                  <span className="text-slate-800">{intern.user?.telephone || '—'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Département</span>
                  <span className="text-slate-800">{intern.department?.name || '—'}</span>
                </div>
                <div className="flex justify-between">
