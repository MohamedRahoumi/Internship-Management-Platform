...
const Interns = () => {
  ...
  const handleExport = async () => {
    try {
      const res = await api.get('/export/interns/excel', { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement('a');
      a.href = url;
      a.download = 'stagiaires-' + new Date().toISOString().split('T')[0] + '.xlsx';
  ...
  return (
    <div>
      <div className={fadeCls(0)}>
        <h1 className="font-cooper-medium text-3xl sm:text-4xl text-ocp-700 tracking-tight">Stagiaires</h1>
        <p className="text-gray-500 text-sm mt-1">Liste des stagiaires inscrits</p>
      </div>
      ...
      <div className={`flex flex-col sm:flex-row gap-3 mt-8 ${fadeCls(1)}`}>
        ...
        <button onClick={handleExport}
          className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-ocp-500 text-white text-sm font-semibold hover:bg-ocp-600 transition-all whitespace-nowrap">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr>
                  ...
                  <th className="text-left px-5 py-3.5 font-semibold text-gray-500 text-[0.7rem] uppercase tracking-wider border-b border-gray-200">Superviseur</th>
                ...
                {interns.map((intern) => {
                  ...
                  return (
                    <tr key={intern.id} className="hover:bg-ocp-100 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-gray-600 border-b border-black/3">{intern.supervisor ? `${intern.supervisor.prenom} ${intern.supervisor.nom}` : '—'}</td>
